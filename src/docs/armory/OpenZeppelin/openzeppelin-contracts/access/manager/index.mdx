...
```
| selector | restricted | role | delay |
|---|---|---|---|
| `labelRole` `setRoleAdmin` `setRoleGuardian` `setGrantDelay` `setTargetAdminDelay` | true | ADMIN_ROLE  | 0 |
|   `updateAuthority` `setTargetClosed` `setTargetFunctionRole` | true | ADMIN_ROLE | getTargetAdminDelay(target) |
|   `grantRole` `revokeRole`    | true | getRoleAdmin(roleId)   | 0 |
